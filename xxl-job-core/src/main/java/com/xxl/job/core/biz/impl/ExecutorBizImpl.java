...
public class ExecutorBizImpl implements ExecutorBiz {
    ...
    public ReturnT<String> run(TriggerParam triggerParam) {
        ...
        // 如果为 BEAN 模式，就通过定时任务的名字，从 jobHandlerRepository 这个 Map 中获取 JobHandler
        if (GlueTypeEnum.BEAN == glueTypeEnum) {
            // 在这里获得定时任务对应的 JobHandler 对象，其实就是 MethodJobHandler 对象
            IJobHandler newJobHandler = XxlJobExecutor.loadJobHandler(triggerParam.getExecutorHandler());
            /*
            这里会进行一下判断，如果上面得到的 JobHandler 并不为空，说明该定时任务已经执行过了，并且分配了对应执行任务
            ...
            if (jobHandler == null) {
                /*
                如果走到这里，就意味着 JobHandler 为 null，这也就意味着上面得到的 jobThread 为 null，这就说明，
                这次调度的定时任务是第一次执行，所以直接让 JobHandler 等于从 jobHandlerRepository 这个 Map 中
                获取 newJobHandler 即可，然后这个 JobHandler 会在下面创建 JobThread 的时候用到
                 */
    ...
    public ReturnT<String> kill(KillParam killParam) {
        // 根据任务 ID 获取到对应的执行任务的线程
        JobThread jobThread = XxlJobExecutor.loadJobThread(killParam.getJobId());
        if (jobThread != null) {
            // 从 Map 中移除该线程，同时也终止该线程
